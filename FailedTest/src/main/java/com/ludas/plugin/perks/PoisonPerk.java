...
public class PoisonPerk extends Perk {
    ...
    public static final BuilderCodec<PoisonPerk> ENTITY_CODEC;
    ...
    public void unlockCondition(float dt, int idx, @NonNullDecl ArchetypeChunk<EntityStore> archetypeChunk,
        ...
        if(magnumOpus.getStat(MagnumOpusStatTypes.UNKNOWN.id).getLevel().getCurrentLevel() <= 1) return;
        TestPlugin.LOGGER.atInfo().log(""+magnumOpus.getStat(MagnumOpusStatTypes.UNKNOWN.id).getLevel().getCurrentLevel());
        this.setUnlocked();
        magnumOpus.getStat(MagnumOpusStatTypes.UNKNOWN.id).putPerk(this);
    ...
    static {
        ...
        ENTITY_CODEC = BuilderCodec.builder(PoisonPerk.class, PoisonPerk::new).build();
    }
}
